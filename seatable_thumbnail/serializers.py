...
from seatable_thumbnail.utils import get_file_id, parse_django_session


def get_enable_thumbnail_file_type():
    ...
    return enable_file_type


ENABLE_THUMBNAIL_FILE_TYPE = get_enable_thumbnail_file_type()
...
class ThumbnailSerializer(object):
    ...
    def params_check(self):
        ...
        if file_type not in ENABLE_THUMBNAIL_FILE_TYPE:
    ...
    def session_check(self):
        ...
        self.session_data = parse_django_session(django_session.session_data)
...
class PluginSerializer(object):
    ...
    def params_check(self):
        ...
        content_type = mimetypes.guess_type(path)[0] or TEXT_CONTENT_TYPE.decode('utf-8')
...
class AssetSerializer(object):
    ...
    def gen_asset_info(self):
        asset_info = {}
        asset_info.update(self.params)
        asset_info.update(self.session_data)
        asset_info.update(self.resource)
        self.asset_info = asset_info

    def params_check(self):
        file_path = '/' + self.request.url.split('/', 2)[-1]
        file_name = os.path.basename(file_path)
        file_ext = os.path.splitext(file_name)[1][1:].lower()
        file_type = FILE_EXT_TYPE_MAP.get(file_ext, 'Unknown')
        ...
        if not all([
            url_split[0] == 'workspace',
            url_split[2] == 'asset',
        ]):
            raise AssertionError(400, 'url invalid.')

        #
        workspace_id = int(url_split[1])
        dtable_uuid = uuid.UUID(url_split[3]).hex
